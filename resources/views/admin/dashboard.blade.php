...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <a href="{{ route('admin.tables.index') }}" class="card hover:shadow-lg transition text-center border-2 border-purple-200">
            <svg class="h-12 w-12 text-purple-600 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
            </svg>
            <p class="font-semibold">Tables (Action)</p>
        </a>
    </div>
    ...
    <!-- État des Tables (Direct) - Now at the bottom -->
    <div class="card mb-6">
        <h2 class="text-xl font-semibold mb-4">État des Tables (Direct)</h2>
        <div id="admin-table-grid">
            @include('partials.tables_grid', ['tables' => $tables])
        </div>
    </div>

</div> <!-- Close Content Div if needed?? No, content div is in Layout -->
@endsection

@section('scripts')
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const gridWrapper = document.getElementById('admin-table-grid');
        setInterval(() => {
            fetch('{{ route('admin.tables.refresh') }}')
                .then(response => response.text())
                .then(html => {
                    gridWrapper.innerHTML = html;
                })
                .catch(err => console.error('Erreur refresh tables admin:', err));
        }, 2000); // 2 secondes pour temps réel fluide
    });
</script>
@endsection
